...
class JdbcTemplateStorageAccessor extends AbstractStorageAccessor {
    ...
    private static boolean isNeedApp = false;
    ...
    JdbcTemplateStorageAccessor(@NonNull Configuration configuration) {
        ...
        // 补充创建标志位
        isNeedApp = ( null==configuration.getTableAppName() || "".equals(configuration.getTableAppName().trim()) )?false:true;
    }
    ...
    public boolean insertRecord(@NonNull LockConfiguration lockConfiguration) {
        try {
            ...
            logger.debug("shedlock插入语句:{},{}",sql,lockConfiguration);
            if( isNeedApp && !isAlreadyCreatedApp ){
                // synchronized is for oracle DB
                synchronized (this) {
