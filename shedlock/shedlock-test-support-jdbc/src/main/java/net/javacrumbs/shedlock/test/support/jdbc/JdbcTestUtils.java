...
public final class JdbcTestUtils {
    ...
    public Timestamp getLockedUntil(String lockName) {
        return jdbcTemplate.queryForObject("SELECT lock_until FROM shedlock WHERE application = ? and name = ?", Timestamp.class, LockConfiguration.getSchedName(),lockName);
    }

    public LockInfo getLockInfo(String lockName) {
        return jdbcTemplate.query("SELECT name, lock_until, " + dbConfig.nowExpression() + " as db_time FROM shedlock WHERE application = ? and name = ?", (rs, rowNum) -> new LockInfo(
            rs.getString("name"),
            rs.getTimestamp("lock_until").toInstant(),
            rs.getTimestamp("db_time").toInstant()
        ), LockConfiguration.getSchedName(),lockName).get(0);
    }
